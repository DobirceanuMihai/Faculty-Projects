...
public class MatrixOperations {
	 ...
	 *
	 * @param matriceDet matrix of which we need to know determinant
	 *
	 * @return determinant of given matrix
	 */
	public static double matrixDeterminant (double[][] matriceDet) {
		double temporary[][];
		double result = 0;

		if (matriceDet.length == 1) {
			result = matriceDet[0][0];
			return (result);
		}

		if (matriceDet.length == 2) {
			result = ((matriceDet[0][0] * matriceDet[1][1]) - (matriceDet[0][1] * matriceDet[1][0]));
			return (result);
		}

		for (int i = 0; i < matriceDet[0].length; i++) {
			temporary = new double[matriceDet.length - 1][matriceDet[0].length - 1];

			for (int j = 1; j < matriceDet.length; j++) {
				for (int k = 0; k < matriceDet[0].length; k++) {
					if (k < i) {
						temporary[j - 1][k] = matriceDet[j][k];
					} else if (k > i) {
						temporary[j - 1][k - 1] = matriceDet[j][k];
					}
				}
			}

			result += matriceDet[0][i] * Math.pow (-1, (double) i) * matrixDeterminant (temporary);
		}
		return (result);
	}
